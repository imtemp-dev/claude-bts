...
스펙 단계에서 bts는 문서를 반복합니다 — 의도를 탐색하고, 코드베이스를 조사하고, 블루프린트를 작성하고, 별도 AI 컨텍스트에서 여러 라운드의 검증을 거칩니다. 여기서 잡힌 에러는 텍스트 수정 비용입니다.

**블루프린트는 전사(轉寫)가 아닙니다.** 코드가 값싸게 반증할 수 없는 것만 담습니다 — 불변식과 그것을 지키는 파일, 경계를 넘나드는 것의 모양, 되돌릴 수 없는 순서와 그것을 되돌리는 방법, 각 불변식을 거짓으로 만들 테스트의 이름. 함수 시그니처·타입 정의·엣지 케이스 표는 일부러 뺍니다: 컴파일러가 공짜로 만들어내는 것을 산문으로 다투면 검증 라운드가 하나씩 들고, 다음 라운드가 다시 검사해야 할 주장만 남습니다.

검증 루프에는 상한이 있습니다 — 라운드 상한과 무진전 예산. 어느 쪽에 닿든 남은 질문은 `## Known Uncertainties`로 옮기고 구현으로 넘어갑니다. 실행되는 코드가 몇 초 만에 정리해줍니다.
...
- **블루프린트는 전사가 아니다** — 코드가 값싸게 반증할 수 없는 것만 문서에 남긴다
- **루프에는 상한이 있다** — 수렴하지 않으면 남은 질문을 Known Uncertainties로 옮기고 구현한다
