#!/usr/bin/env tsx
// Capture a snapshot of a jig-managed project for regression monitoring.
//
// Usage:
//   tsx scripts/jig-baseline.ts --target <path> --out <file.json>
//
...
// Output is structured JSON so later runs (scripts/jig-monitor.ts) can
...
function parseArgs(): { target: string; out: string } {
  ...
  if (!target) {
    console.error('Usage: jig-baseline.ts --target <path> --out <file.json>');
    process.exit(2);
  }
  if (!out) {
