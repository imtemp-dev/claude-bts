...
仕様フェーズでは、btsはドキュメントを反復します — インテントの探索、コードベースの調査、ブループリントの作成、独立したAIコンテキストでの複数ラウンドの検証。ここで発見されたエラーはテキスト編集で修正できます。

**ブループリントは転写ではありません。** コードが安価に反証できないものだけを載せます — 各不変条件とそれを守るファイル、境界を越えるものの形、取り消せない順序とその戻し方、各不変条件を偽だと示すテストの名前。関数シグネチャ・型定義・エッジケース表は意図的に外します: コンパイラがただで生成するものを散文で論じると検証ラウンドを1つずつ消費し、次のラウンドが再検査すべき主張だけが残ります。

検証ループには上限があります — ラウンド上限と無進捗バジェット。どちらかに達したら、残る問いは `## Known Uncertainties` へ移して実装に進みます。動くコードが数秒で片づけます。
...
- **ブループリントは転写ではない** — コードが安価に反証できないものだけをドキュメントに残す
- **ループには上限がある** — 収束しなければ残る問いを Known Uncertainties に移して実装に進む
