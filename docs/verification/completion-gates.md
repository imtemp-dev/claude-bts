...
```
$ bts recipe log r-ev --iteration 1 --critical 2 --doc draft.md
...
$ bts recipe log r-ev --iteration 2 --critical 2 --doc draft.md
[bts] note: verify.max_iterations changed 3 → 6 since the last round of draft.md. Earlier rounds were judged under the old budget; the convergence verdict is recomputed from the current one.
...
$ cat .bts/specs/recipes/r-ev/verify-log.jsonl
...
ok  	github.com/imtemp-dev/claude-bts/internal/state	0.178s
```

## G2 — A turn cannot end with the recipe's own records inconsistent

Before this, the Stop gate ran only on an explicit `<bts>DONE</bts>` marker;
...
ok  	github.com/imtemp-dev/claude-bts/internal/hook	0.413s
```

Loop bound: the same block reason may fire at most
`state.DefaultStopBlockBudget` (3) times consecutively, below Claude Code's
own 8-block override, after which bts stands down with a visible message
...
reported by `bts doctor`, and shows in the status line; and the ledger
refuses both a key collision and a fabricated answer.

```
$ bts recipe decision hold r-smoke --key token-storage \
...
Decision "token-storage" held. r-smoke is blocked until it is resolved:
  bts recipe decision resolve r-smoke token-storage --answer "..."

$ bts recipe decision list r-smoke
...
# original question.
$ bts recipe decision hold r-smoke --key token-storage --question "a different question"
...
$ bts recipe decision resolve r-smoke token-storage --answer "httpOnly cookie"
Decision "token-storage" resolved: httpOnly cookie

$ cat .bts/specs/recipes/r-smoke/decisions.jsonl
...
orchestrator writes `verification.md` and runs `bts recipe log`, and nothing
connected that write to a fork having run.

Guarantee: each round records whether a subagent finished since the previous
round, and `bts doctor` reports an evidence-free round **only** when the same
...
```
$ go test -race ./...
?   	github.com/imtemp-dev/claude-bts/cmd/bts	[no test files]
ok  	github.com/imtemp-dev/claude-bts/internal/cli	3.069s
ok  	github.com/imtemp-dev/claude-bts/internal/comment	(cached)
ok  	github.com/imtemp-dev/claude-bts/internal/engine	1.493s
ok  	github.com/imtemp-dev/claude-bts/internal/hook	3.455s
ok  	github.com/imtemp-dev/claude-bts/internal/metrics	2.868s
ok  	github.com/imtemp-dev/claude-bts/internal/state	2.599s
?   	github.com/imtemp-dev/claude-bts/internal/statusline	[no test files]
ok  	github.com/imtemp-dev/claude-bts/internal/template	(cached)
?   	github.com/imtemp-dev/claude-bts/pkg/version	[no test files]
```
