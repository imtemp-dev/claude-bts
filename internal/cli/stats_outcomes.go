...
func renderOutcomes(outs []RecipeOutcome) string {
	...
	fmt.Fprintln(w, "ID\tType\tPhase\tVerifyIter\tFirst C/M\tSimRuns\tTasks\tRetries\tBlocked\tTestIter\tTest\tDeviations")
	for _, o := range outs {
		test := o.TestStatus
		if test != "" && o.TestRecordedBy != "bts" {
			test += " (hand-recorded)"
		}
		fmt.Fprintf(w, "%s\t%s\t%s\t%d\t%d/%d\t%d\t%d\t%d\t%d\t%d\t%s\t%d\n",
			o.ID, o.Type, o.Phase, o.VerifyIterations, o.FirstCritical, o.FirstMajor,
			o.SimulateRuns, o.Tasks, o.ImplRetries, o.BlockedTasks,
			o.TestIterations, test, o.Deviations)
	}
	w.Flush()

	// Grouped means over recipes that actually reached implementation.
