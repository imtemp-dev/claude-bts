...
// `jig validate --gates` cross-checks that every declared hard gate maps
// to a real code path.
...
// here will fail `jig validate --gates` once that subcommand ships.
var HardGates = []HardGate{
	{
		ID:          "verify_after_modification",
		Rule:        "jig-recipe-protocol.md §Mandatory Rules rule 3",
		Enforcement: "internal/hook/stop.go:handleSpecDone",
		Summary:     "Block <jig>DONE</jig> unless verify-log last entry has critical=0 major=0 minor_resolvable=0",
	},
	{
		ID:          "log_every_action",
		Rule:        "jig-recipe-protocol.md §Mandatory Rules rule 4",
	...
	},
	{
		...
		Rule:        "jig-recipe-protocol.md §Mandatory Rules rule 5",
		Enforcement: "internal/hook/stop.go:handleSpecDone (spec: changelog must contain a simulate action) + internal/cli/recipe.go:checkPhasePreConditions(review) warn",
		Summary:     "Block <jig>DONE</jig> for spec recipes whose changelog has no simulate action",
	},
	{
		ID:          "adjudicate_every_debate",
		Rule:        "jig-recipe-protocol.md §Mandatory Rules rule 7",
		Enforcement: "internal/engine/validator.go:validateDebates + internal/cli/doctor_drift.go:checkOrphanedProjectDebates",
		Summary:     "Every debate must carry a state file (debate.json) with a decided boolean and, when decided, a string conclusion — validated per debate ID across the recipe and project trees, and by `jig doctor` for project-tree debates that belong to no recipe",
	},
	{
		ID:          "sync_check_before_final",
		Rule:        "jig-recipe-protocol.md §Mandatory Rules rule 8",
		Enforcement: "internal/cli/sync_check.go (changelog append) + internal/hook/stop.go:handleSpecDone (pass-after-last-modification ordering gate, spec)",
		Summary:     "Block <jig>DONE</jig> unless a passing sync-check changelog entry postdates the last draft/improve/comment-apply action",
	},
	{
		ID:          "deferred_minors_declared",
		Rule:        "jig-spec SKILL.md §Quality Rules 3b",
		Enforcement: "internal/hook/stop.go:handleSpecDone (via engine.CheckKnownUncertainties)",
		Summary:     "Block <jig>DONE</jig> when the last verify entry has minor_deferred>0 but the spec carries no ## Known Uncertainties (### U-NNN) entries",
	},
	{
		ID:          "status_at_finalization",
		Rule:        "jig-recipe-protocol.md §Mandatory Rules rule 9",
		Enforcement: "internal/hook/stop.go:handleSpecDone",
		Summary:     "Completion sets phase=finalize which triggers the status flow",
	},
	{
		ID:          "spec_before_code",
		Rule:        "jig-implement-protocol.md §Execution Rules rule 1",
	...
	},
	{
		...
		Rule:        "jig-implement-protocol.md §Execution Rules rule 2",
		Enforcement: "jig-implement/SKILL.md Step 3 + stop hook via test-results.json",
		Summary:     "Implementation skill runs build per task; stop hook requires test-results.status=pass",
	},
	{
		ID:          "test_after_implement",
		Rule:        "jig-implement-protocol.md §Execution Rules rule 4",
	...
	{
		ID:          "sync_after_test",
		Rule:        "jig-implement-protocol.md §Execution Rules rule 5",
		Enforcement: "internal/hook/stop.go:handleImplementDone",
		Summary:     "IMPLEMENT DONE blocked unless deviation.md exists",
	},
	{
		ID:          "review_before_done",
		Rule:        "jig-implement-protocol.md (implied by handleImplementDone)",
		Enforcement: "internal/hook/stop.go:handleImplementDone",
		Summary:     "IMPLEMENT DONE blocked unless review.md exists",
	},
	{
		ID:          "phase_transitions_logged",
		Rule:        "jig-implement-protocol.md §Execution Rules rule 7",
	...
	},
	{
		...
		Rule:        "jig-implement-protocol.md §Execution Rules rule 6",
	...
	},
	{
		...
		Rule:        "jig-implement/SKILL.md §Step 5.7 + §Completion",
		Enforcement: "internal/hook/stop.go:handleImplementDone",
	...
	},
	{
		...
		Rule:        "jig-implement/SKILL.md §Step 1 + jig-schema.md tasks.json",
	...
	},
	{
		...
		Rule:        "jig-implement/SKILL.md §Step 3 IMPLEMENT (Phase 14)",
	...
	},
	{
		...
		Rule:        "jig-sync/SKILL.md §Step 5 (Phase 16)",
	...
	},
	{
		...
		Rule:        "jig-sync/SKILL.md §Step 2.5 (Phase 12)",
	...
	},
	{
		...
		Rule:        "jig-test/SKILL.md §Step 2 + §Step 3 ASSESS (Phase 13)",
		Enforcement: "internal/engine/test_scenario_map.go:CheckTestScenarioCoverage",
		Summary:     "Every simulation scenario must be linked via `jig:scenario {id}` to at least one test; failing results must carry a `category`",
	},
	{
		ID:          "recipe_state_normalized",
		Rule:        "jig-recipe-protocol.md §Completion (Phase 21)",
		Enforcement: "internal/hook/stop.go:handleSpecDone + internal/cli/recipe.go:recipeReconcileCmd",
		Summary:     "On <jig>DONE</jig> recipe.json level→3.0 and iteration←max(curr, last_verify); `jig recipe reconcile` recovers missed DONE",
	},
	{
		ID:          "verification_log_consistency",
		Rule:        "jig-verify/SKILL.md §Output + Phase 22",
		Enforcement: "internal/engine/validator.go:validateVerificationLogConsistency",
		Summary:     "verification.md <jig-findings> counts must match the last verify-log.jsonl entry",
	},
	{
		ID:          "findings_array_consistency",
		Rule:        "jig-verification-protocol.md §Finding Identity",
		Enforcement: "internal/engine/validator.go:ParseFindingsBlock",
		Summary:     "A <jig-findings> findings array must match the block's counts per severity and carry non-empty titles; a mismatch fails `jig recipe log` so a round cannot be recorded with an unusable ledger",
	},
	{
		ID:          "verification_not_passed",
		Rule:        "jig-verification-protocol.md §Completion Evidence",
		Enforcement: "internal/hook/stop.go:handleSpecDone",
		Summary:     "Block <jig>DONE</jig> while the spec document's last verify round still reports critical, major or resolvable-minor findings",
	},
	{
		ID:          "convergence_budget",
		Rule:        "jig-verification-protocol.md §Convergence",
	...
	},
	{
		...
		Summary:     "Block <jig>DONE</jig> when the spec's last verify entry is a scoped delta pass rather than a full-document pass",
	},
	{
		ID:          "measurement_class_comparability",
		Rule:        "jig-verification-protocol.md §Measurement Strength",
	...
	},
	{
		...
		Rule:        "jig-verification-protocol.md §Completion Evidence",
		Enforcement: "internal/engine/completion_evidence.go:EvaluateCompletionEvidence + internal/hook/stop.go:handleSpecDone",
		Summary:     "verify.confirm_passes consecutive qualifying clean rounds on the SAME recorded doc_hash are required before <jig>DONE</jig>; a single clean round is a sample, not a measurement",
	},
	{
		ID:          "revision_recorded_before_final",
		Rule:        "jig-verification-protocol.md §Completion Evidence",
		Enforcement: "internal/engine/completion_evidence.go:qualifies + internal/cli/doctor.go",
		Summary:     "A verify round with no doc_hash cannot be replicated against, so completion blocks and `jig doctor` reports the gap instead of the gate falling open silently",
	},
	{
		ID:          "gate_override_recorded",
		Rule:        "jig-verification-protocol.md §Gate Overrides",
	...
	},
	{
		...
		Summary:     "A finding that stops being reported goes to `unreported`, blocks <jig>DONE</jig> while it stays there, and closes only after a second silent round on an anchor that has stopped producing findings — so a restated finding cannot fold into `fixed` and read as progress",
	},
	{
...
// InvariantGates lists domain-level checks enforced via `jig verify`.
...
var InvariantGates = []HardGate{
	{
		...
		Rule:        "jig-recipe-protocol.md §Mandatory Rules rule 10",
		Enforcement: "internal/cli/recipe.go:checkPhasePreConditions(wireframe)",
		Summary:     "domain.md must exist for spec/design recipes before phase=wireframe (strict)",
	},
	{
		ID:          "invariant_single_owner",
		Rule:        "jig-domain-model/SKILL.md §Quality Gate",
	...
	},
	{
		...
		Rule:        "jig-implement/SKILL.md §Step 3 MID-RUN REVIEW + settings.implement.midrun_review_every",
		Enforcement: "jig-implement/SKILL.md orchestrator (advisory — not hook-blocked)",
	...
	},
	{
		...
		Rule:        "jig-implement/SKILL.md §Step 3 VERIFY retry block (Phase 15)",
		Enforcement: "engine.NextRetryDecision + `jig retry next` CLI (advisory — monitored in Phase 17)",
...
// overridableGates are the hard gates `jig recipe override grant` accepts.
//
...
// jig lie about what was verified, only about whether that was enough.
