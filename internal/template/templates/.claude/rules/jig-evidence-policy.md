...
Single source of truth for how jig skills and agents ground
framework/platform claims. `jig-verify` and `jig-audit` embed the
...
0. **Local evidence cache** — always first. The verification loop
   ...
   - `unavailable` entries expire after **one hour**, so a Context7
     ...
     (default 30).
   - The cache is machine-local (`.jig/local/evidence-cache.jsonl`) and
     never committed. It is a latency optimisation, not project truth —
     when in doubt, `jig evidence prune` and re-gather.
...
recommended architecture, `/jig-research` MUST record it (research doc
`## Official Guidance` section) and `/jig-architect` MUST include it as
...
Guidance must be **version-matched**: `/jig-research` Step 3.5 records
