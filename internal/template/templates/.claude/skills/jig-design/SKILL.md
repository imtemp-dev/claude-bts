...
description: >
  ...
  Can be followed by /jig-spec to reach Level 3.
user-invocable: true
...
```bash
jig recipe create --type design --topic "$ARGUMENTS"
```
Use the output as recipe ID for all subsequent commands.

If active:
- Phase `research` → read existing research doc, continue from Step 2.
- Phase `draft` → read draft, run /jig-assess, then **immediately execute** the recommended action.
- Phase `debate` → read debate state, continue deliberation.
- Phase `verify` → read draft + verification, run /jig-assess, then **immediately execute**.
...
- `.jig/specs/layers/{name}.md` — detail for layers relevant to this design

Use Skill("jig-research") to understand the current state.
...
- Skill("jig-cross-check"): referenced code/systems exist?
- Skill("jig-verify"): design is logically consistent?
- Skill("jig-audit"): missing considerations?
...
1. Use Skill("jig-debate") to deliberate
2. Immediately after debate completes, run Skill("jig-adjudicate") to evaluate
...
3. If debate reaches [DEBATE DEADLOCK] → present each position to user,
   user decides, run Skill("jig-adjudicate") on user's decision
...
- To create a full implementation spec: `/jig-spec "topic"`
  The design final.md provides the Level 2 foundation for Level 3 spec.
- To implement directly (if design is detailed enough): `/jig-implement {id}`
