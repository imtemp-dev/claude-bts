...
0. **Resolve recipe ID**: If `$ARGUMENTS` is empty or not a recipe ID,
   run `jig recipe status` to find the active recipe. Use its ID for
   all `{id}` references below. If no active recipe → "No active recipe. Run /jig-spec first."

1. Verify final.md exists:
   ```bash
   ls .jig/specs/recipes/{id}/final.md
   ```
   If not found → "Run /jig-spec first."
...
2. Verify spec quality gate:
   - Check `verify-log.jsonl` exists and last entry has critical=0,
     ...
     stop hook applied at `<jig>DONE</jig>`; minor_deferred may be > 0)
   - If verify-log is missing or the bar is not met →
     "Spec not verified. Run /jig-spec to complete verification before implementing."
...
3. Check recipe phase:
   ...
   - If phase is "test" → smart resume based on artifacts.
     ...
     document-mode simulations from the spec phase always live
...
4. **Load design context**:
   ...
   - Read project-map.md (at `.jig/specs/project-map.md`) for layer-specific
...
1. **Anchor staleness** (Phase 9): run `jig validate`. If `task_anchor`
...
1. Read `.jig/specs/recipes/{id}/final.md`
...
4. **Anchor every decomposition** (Phase 9 contract): for each task you
   ...
   its comment verbatim. `jig verify` enforces a 1:1 mapping — a missing
   ...
   **Modify tasks declare scope at decomposition time (Phase 14).**
   ...
   scope raises `modify_scope_required` (major) at `jig validate`;
   declared symbols absent from the target file raise
   `scope_symbol_missing` (critical) at IMPLEMENT DONE.

5. Save task list to `.jig/specs/recipes/{id}/tasks.json`:
   ```json
   {
     "recipe_id": "{id}",
     "started_at": "ISO8601",
...
6. Update phase and log:
   ```bash
   jig recipe log {id} --phase implement --action implement --output tasks.json --based-on final.md --result "N tasks decomposed"
   ```
...
**Reservations check**: If `.jig/specs/recipes/{id}/reservations.md` exists,
...
2. Ask the engine what to do next:
   ```bash
   jig retry next --recipe {id} --task {task-id} --json
   ```
...
3. Execute the returned action:
   ...
   - `spec_escalate`    — re-read the final.md block for this file,
     run `/jig-verify` focused on it. If the block needs additions,
     edit final.md (and re-validate the anchor).
   - `domain_escalate`  — run `/jig-verify domain.md` and confirm the
     invariant ownership for this file has not drifted. If it has,
     reconcile before continuing.
   - `architect_escalate` — invoke `/jig-architect escalate {id} {task-id}`
...
```bash
jig check task --recipe {id} --task {task-id} --write
```
...
Interpret results:
  ...
  - `jig check task` exits 2 on critical, so shells checking `$?`
    see the failure.
...
if settings.implement.midrun_review_every > 0 AND
   completed_since_last_midrun >= settings.implement.midrun_review_every:
    ...
    Use Skill("jig-review") with arguments: "--mid-run {window}"
...
### Log Each Task
```bash
jig recipe log {id} --action implement --result "task {task-id} done"
```
...
```bash
jig recipe log {id} --phase test
```

Use Skill("jig-test") with arguments: {id}
The test skill will read final.md for test scenarios and tasks.json
for the list of implemented files.

**If tests fail** (jig-test does not output `<jig>TESTS PASS</jig>`):
...
Use Skill("jig-simulate") with arguments: "code" to verify all code paths are covered.
...
- Re-run tests: use Skill("jig-test") with arguments: {id}
  (jig-test skips generation, re-runs tests, updates test-results.json)
- If tests fail → fix and let jig-test retry loop handle it
- Do NOT re-run simulation (runs once per implementation)

If no gaps → proceed to Step 5.5 (Review).

```bash
jig recipe log {id} --action simulate --result "N scenarios, N gaps"
```
...
Update phase:
```bash
jig recipe log {id} --phase review
```

Use Skill("jig-review") (full mode, no arguments — uses tasks.json for scope).
...
- After all fixes, re-run tests: use Skill("jig-test") with arguments: {id}
  (jig-test skips generation, re-runs tests, updates test-results.json)
- If tests fail → fix and let jig-test retry loop handle it
...
```bash
jig recipe log {id} --action review --output review.md --result "N critical, N major (N actionable)"
```
...
Log:
```bash
jig recipe log {id} --action resolve-uncertainties --result "N resolved, N diverged, N still-unknown"
```

Diverged entries stay inline in final.md's Known Uncertainties section as
documentation of what changed during implementation. `jig-sync` compares
...
```bash
jig recipe log {id} --phase sync
```

Use Skill("jig-sync") with arguments: {id}

## Step 7: Status

After sync:
```bash
jig recipe log {id} --phase status
```

Use Skill("jig-status") with arguments: {id}
...
- Tell the user (plaintext, after the marker):
  ...
  > Next: run `/jig-spec` to start the next roadmap item, or `/jig-fix` for any bugs.
