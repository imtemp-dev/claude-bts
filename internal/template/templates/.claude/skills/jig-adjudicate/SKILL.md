...
- List `.jig/specs/recipes/{id}/debates/` to find the latest debate
...
### 2b. Scope Compliance
- Read `.jig/specs/recipes/{id}/scope.md`
...
- **Source hierarchy** (per `.claude/rules/jig-evidence-policy.md`):
...
2. Save preparation results to `.jig/specs/recipes/{id}/debates/{debate-id}/prep-roundN.md`

3. Log to changelog:
   ```bash
   jig recipe log {id} --action adjudicate --result "extend: [reasons]"
   ```
...
4. Kick off a follow-up debate:
   ...
   - Use Skill("jig-debate") with this combined context as arguments
...
`.jig/specs/recipes/{id}/debates/` that contain "-followup" in their name.
...
**Save reservations** to `.jig/specs/recipes/{id}/reservations.md`:
```markdown
...
Log:
```bash
jig recipe log {id} --action adjudicate --output reservations.md --result "accepted with reservations after max extensions"
```

## Output

Always output one of:
- `<jig>ADJUDICATE: ACCEPT</jig>` — conclusion is solid, proceed
- `<jig>ADJUDICATE: EXTEND N/3</jig>` — another round needed, preparation brief attached
- `<jig>ADJUDICATE: ACCEPT WITH RESERVATIONS</jig>` — max extensions reached, proceed with caveats
