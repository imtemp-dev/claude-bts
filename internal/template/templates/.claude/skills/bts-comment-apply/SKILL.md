---
name: bts-comment-apply
description: >
  Incorporate inline BTS review comments embedded in a recipe's docs.
  Reads `.bts/local/recipes/{id}/pending-comments.json` (written by
  `bts comment apply`), runs meta-analysis (conflicts, cascades,
...
Incorporate BTS review comments for recipe: $ARGUMENTS

This skill is the second half of the comment workflow. The first half
(`bts comment apply <id>`) parses inline `> [!BTS-COMMENT]`,
`> [!BTS-BLOCK]`, `> [!BTS-Q]` callouts and writes a structured handoff
to `.bts/local/recipes/{id}/pending-comments.json`. This skill reads that
handoff, edits the recipe docs, then calls `bts comment apply --finalize`
...
```bash
cat .bts/local/recipes/{id}/pending-comments.json
```

If the file does not exist, tell the user to run `bts comment apply {id}`
first and stop.
...
```bash
STASH_MSG="bts-comment-apply {id} $(date +%s)"
git stash push --include-untracked -m "$STASH_MSG" -- .bts/specs/recipes/{id} >/dev/null 2>&1
# Locate the stash by message — empty if nothing was pushed
STASH_REF=$(git stash list | grep -F "$STASH_MSG" | head -1 | cut -d: -f1)
echo "$STASH_REF" > .bts/local/recipes/{id}/comment-apply.stash-ref
```
...
```bash
REF=$(cat .bts/local/recipes/{id}/comment-apply.stash-ref 2>/dev/null)
...
```
You are reviewing a batch of inline review comments for a BTS recipe and
...
## Source docs
{list of all .md files under .bts/specs/recipes/{id}/ — read each before
analyzing}
...
`questions` enumerates every BTS-Q callout — Pass A does NOT answer them
(that's the user's job in the resolution loop). Including them here makes
the resolution loop self-contained: every BTS-Q gets explicitly asked,
nothing falls through to "TODO" silently.

If a category is empty, return an empty list. No prose, no markdown — just JSON.
```

Save the returned JSON to `.bts/local/recipes/{id}/meta-analysis.json`.
...
ran `bts comment apply --include-freeform`), there is no marker to
...
tell them to fix the comments and re-run `bts comment apply`.
...
```
You are editing a BTS spec document to incorporate review comments.

## Doc to edit
.bts/specs/recipes/{id}/{doc}
...
1. For each comment, edit the doc to incorporate the change. Use the
   ...
   block itself (`> [!BTS-COMMENT]` etc.) is the exact location.
2. After editing, REMOVE the entire callout block (the marker line plus
   all body lines). Do NOT leave the marker behind.
3. For BTS-Q (questions), do NOT auto-edit the body. Instead, append a
...
```bash
git diff --color=always .bts/specs/recipes/{id}
```
...
rm -f .bts/local/recipes/{id}/comment-apply.stash-ref
```
...
if [ -n "$REF" ] && git stash list | grep -q "^$REF:"; then
  ...
  git checkout -- .bts/specs/recipes/{id}
fi
rm -f .bts/local/recipes/{id}/comment-apply.stash-ref
```
Then abort.
...
>
> If L > 0, the recipe still cannot finalize. Run `bts comment list {id}`
> to see what's left.
...
- Never call `bts comment apply --finalize` until the user explicitly
  accepted in Pass C.
