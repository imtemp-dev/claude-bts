...
If `agents.simulator` is explicitly set in `.jig/config/settings.yaml`, use that model instead.
...
rebuttal round. Override in `.jig/config/settings.yaml`. Rebuttal uses the session
...
`jig validate` parses the simulation file and raises critical if the
...
shape so `jig validate` can count tags consistently.
...
If `jig validate` reports `no_scenarios_detected` despite the file
...
**DEVIATIONs are excluded from adversarial validation** — they go to jig-sync
...
Save to `.jig/specs/recipes/{id}/simulations/NNN-code.md`
...
### DEVIATIONs (for jig-sync)
- [DEVIATION-001] {description}
```

Log:
```bash
jig recipe log {id} --action simulate --result "N scenarios, N gaps (N critical), N dismissed"
```
...
3. Re-run tests: use Skill("jig-test") (mandatory after fixes)
4. Route DEVIATIONs to jig-sync for spec reconciliation
...
3.5 **Canonical format + tags (REQUIRED — Code mode Step 3.5 applies to
   document simulations too).** `jig validate` parses EVERY
...
7. Adversarial Validation (Document Mode):
   ...
   **Fallback**: If a validator or rebuttal agent fails (error, timeout), skip
   ...
   .jig/specs/layers/, and the codebase (if it exists). The spec document
   ...
   - .jig/specs/project-map.md (if exists)
   - .jig/specs/layers/ (layer spec files, if any)
...
8. Save simulation results to `.jig/specs/recipes/{id}/simulations/NNN-[category].md`

   Report header should include: `Validation: adversarial (2-round debate)`

9. Log in changelog:
   ```bash
   jig recipe log {id} --action simulate --gaps N
   ```
