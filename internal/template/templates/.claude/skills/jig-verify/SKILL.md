...
If `agents.verifier` is explicitly set in `.jig/config/settings.yaml`, use that model instead.

Bash in this fork is ONLY for the two read-only commands in steps 1-2.
Never run state-mutating jig commands (log, create, finalize, …) or
...
2. Get focus hints from changes since the last verified revision:
   ```bash
   jig recipe verify-focus $ARGUMENTS
   ```
...
2b. Get the adjudicated findings from previous rounds:
   ...
   see `jig-verification-protocol.md § Finding Identity`.

2c. **Decide this round's scope** (`jig-verification-protocol.md §
...
3. Spawn Agent(verifier) with the following prompt, appending
   ...
   **Flow-level verification (mermaid diagrams):**
   ...
   prompt — jig computed it; its path enumeration is AUTHORITATIVE.
   ...
   **Evidence policy for framework/platform claims**
   ...
   0. Cache first — the same claims recur every round, and network
      ...
      ```bash
      jig evidence put --library <lib> --topic <topic> --claim "<claim>" \
   ...
   **Structured findings block (REQUIRED, exact format):**
   ...
   inside. `jig validate` and the stop hook parse this block; numbers in
   ...
   - `severity` is one of `critical`, `major`, `minor_resolvable`,
     `minor_deferred`, `info` — it must match the counts. `jig` rejects
...
The counts embedded in the `<jig-findings>` block MUST match the most
recent `verify-log.jsonl` entry. `jig validate` cross-checks the two —
...
1. The verifier agent returns its findings, `<jig-findings>` block first.
...
3. The orchestrator records the verify-log entry atomically, passing
   ...
   **The `--doc` path must resolve.** A bare `draft.md` is resolved
   ...
   `jig recipe log` warns on stderr when it could not read the file —
   ...
   is what `<jig>DONE</jig>` asks for; see
...
Migrate-seeded blocks (produced by `jig migrate verification`) carry
