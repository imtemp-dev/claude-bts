...
Read `.jig/config/settings.yaml` for `debate.expert_count` (default: 3)
...
`.claude/rules/jig-evidence-policy.md` (Context7 → official docs →
...
an uncited claim is an opinion, and /jig-adjudicate scores it as weak
...
```bash
jig debate log --topic "$ARGUMENTS" --round N --content "round summary"
```

To resume a previous debate:
```bash
jig debate resume <id>
```
