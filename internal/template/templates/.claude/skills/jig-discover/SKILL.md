...
jig recipe status
```
If found, read `.jig/specs/recipes/{id}/intent.md`:
- Status EXPLORING → continue the conversation from where it left off
- Status CONFIRMED → skip discovery, proceed to spec
...
Use this ID for all subsequent `jig recipe log` calls.
...
Update `.jig/specs/recipes/{id}/intent.md` with the current understanding.
...
4. Log:
   ```bash
   jig recipe log {id} --phase discovery --action discover --output intent.md --result "intent confirmed"
   ```

The recipe stays in phase `discovery` until spec advances it.
