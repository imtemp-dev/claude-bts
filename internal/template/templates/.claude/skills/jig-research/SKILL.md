...
1. Spawn Agent(Explore) to investigate the codebase:
   ...
   ```
   If `.jig/specs/recipes/` contains completed recipes, read their
...
3. If additional external research is needed, use WebSearch/WebFetch for:
   ...
   Follow `.claude/rules/jig-evidence-policy.md` for source hierarchy
   ...
   uncited claims force /jig-verify to re-gather the same evidence
...
3.5 **Official Architecture Guidance (MANDATORY when the feature
   ...
   `jig-evidence-policy.md` for examples: Android Guide to App
   ...
   **Pin target versions FIRST**: read the project's dependency
   ...
   This section is the required input for `/jig-architect` — one of its
...
6. Save to `.jig/specs/recipes/{id}/research/v1.md` if inside a recipe
   (increment to v2.md, v3.md… for follow-up research rounds). This is
   the path every downstream skill reads — spec resume, wireframe,
