...
description: >
  ...
  state cells. Produces domain.md as the contract /jig-wireframe and
  /jig-architect must honor.
user-invocable: true
...
- Phase already `domain-model` and `domain.md` exists → read it, continue
  refining if sections are incomplete; else advance to `/jig-architect`
  (or straight to `/jig-wireframe` if wireframe.md already carries an
  `<!-- architect-decision -->` block).
- Phase is `research` or later without `domain.md` → start from Step 1.
- No active recipe → fail: this skill runs inside an active spec
  or design recipe only.

## Prerequisites

1. Read `.jig/specs/recipes/{id}/scope.md` — scope MUST be Status: CONFIRMED
2. Read `.jig/specs/recipes/{id}/research/v1.md` (or latest research doc)
3. Read `.jig/specs/recipes/{id}/intent.md` for the Success Criteria
4. Read `.jig/specs/project-map.md` for existing layer structure

## Output: `.jig/specs/recipes/{id}/domain.md`
...
**concepts**, not files or modules — those come later in /jig-wireframe.
...
`/jig-simulate` Phase 6.2 — the illegal cells above must have
...
## Quality Gate (before proceeding to /jig-wireframe)
...
- [ ] `jig verify .jig/specs/recipes/{id}/domain.md` returns 0 critical, 0 major

## Completion

```bash
jig recipe log {id} --phase domain-model --action domain-model \
