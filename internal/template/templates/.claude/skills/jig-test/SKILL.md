...
3. **Read simulations/*.md (Phase 13)**: collect every scenario id
   ...
   link via `jig:scenario` tags. `jig check test-coverage` enforces
...
5. **Tag every test with `jig:scenario {id}` (Phase 13 — required).**
   ...
   `func TestXxx(t *testing.T)`. Python / pytest uses `# jig:scenario`
   directly above `def test_xxx():`. JS / TS uses `// jig:scenario`
   ...
   `jig validate` (critical for cross-boundary / illegal-cell
...
Test files go in the project's test directory (not in `.jig/`).
...
Repeat the following (max `settings.implement.max_test_iterations` — authoritative source is `.jig/config/settings.yaml`; do NOT hardcode a default here):

### RUN
Execute the test suite via the CLI so status is machine-truthful:
```bash
# Detect the project's test command, then run it THROUGH jig:
jig test run {id} --cmd "go test ./..."
# Node.js: --cmd "npx jest" / "npx vitest run" / "npm test"
# Python:  --cmd "pytest"        Rust: --cmd "cargo test"
```
`jig test run` executes the command itself, streams the output, and
writes test-results.json with `status` derived from the ACTUAL exit
code (`recorded_by: "jig"`), auto-incrementing `iterations`. It exits
non-zero on failure. Do NOT run the test command via plain Bash and
hand-write test-results.json — the DONE gates trust the
machine-recorded status, and `jig doctor` flags hand-recorded files.
Full output is also saved to `.jig/local/recipes/{id}/test-output.log`.
...
### RUN again
- Re-run via `jig test run {id} --cmd ...` after fixes (iterations
...
1. The final `jig test run` invocation already wrote the machine-truth
   core of `.jig/specs/recipes/{id}/test-results.json`: `status`,
   `exit_code`, `iterations`, `command`, `recorded_by: "jig"`.
...
2. Supplement the SAME file with the descriptive fields via the Edit
   ...
   run, since each `jig test run` resets annotations):
   ```json
   {
     "total": 15,
     "passed": 15,
...
2. Log to changelog and manifest:
   ```bash
   jig recipe log {id} --action test --output test-results.json --based-on tasks.json --result "N/N passed"
   ```
...
If all tests pass → output `<jig>TESTS PASS</jig>`
