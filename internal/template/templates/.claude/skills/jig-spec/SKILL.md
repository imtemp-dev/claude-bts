...
Only create documents in `.jig/specs/recipes/{id}/`.
Code implementation happens in `/jig-implement` AFTER this recipe completes with `<jig>DONE</jig>`.

## Settings

Read `.jig/config/settings.yaml` for project-specific limits.
...
1. If `.jig/specs/vision.md` exists with Status: DRAFT → re-present vision for confirmation.
   After vision confirmed, check roadmap below.
2. If `.jig/specs/vision.md` CONFIRMED but `.jig/specs/roadmap.md` missing →
   go to Vision & Roadmap Check step 3b (create roadmap from confirmed vision).
3. If `.jig/specs/roadmap.md` exists with Status: DRAFT → re-present roadmap for confirmation.
...
```bash
jig recipe assess-precheck {id} --doc .jig/specs/recipes/{id}/draft.md
```
Execute the printed `<jig-decision>` if there is one; only run
`/jig-assess` when the precheck reports `UNDECIDED`. Resuming is exactly
...
1. Check `jig recipe status`. If no active recipe exists:
   ```bash
   jig recipe create --type spec --topic "$ARGUMENTS"
   ```
   This creates `recipe.json` and `manifest.json` automatically and outputs the recipe ID.
2. Run `jig validate` to confirm schema compliance

**ALWAYS after modifying any JSON file in .jig/:**
1. Run `jig validate` to verify schema compliance. Fix any errors before continuing.
...
2. Log the action to changelog:
   ```bash
   jig recipe log {id} --action [draft|improve] --output draft.md
   ```
...
4. Run `jig validate` to verify schema compliance
5. Run the semantic pass on draft.md. When both /jig-verify and
   /jig-audit are due this round, **invoke them in a single message so
   ...
   state. Do NOT run them one after the other. Once critical=0, /jig-simulate
...
7. Record verify results to verify-log (atomic — parses the `<jig-findings>`
   ...
   completion replication check; `jig recipe log` warns on stderr when
   that happens. `--scope` records whether
   the round covered the whole document or only the changed sections
   plus their reference closure (`jig-verification-protocol.md §
   ...
   (`jig-verification-protocol.md § Measurement Strength`). Omitting the
...
8. Ask state before asking a model:
   ...
   jig recipe assess-precheck {id} --doc .jig/specs/recipes/{id}/draft.md
   ```
   - Prints a `<jig-decision>` (exit 0) → execute that action directly.
...
**Refer to `.claude/rules/jig-schema.md` for exact JSON field names, types, and structures.**

### Intent Check (before vision/roadmap/scoping)

Before anything else, check if the intent is clear:

1. If `.jig/specs/recipes/{id}/intent.md` exists with Status: CONFIRMED → proceed.
...
3. If no intent.md → run Skill("jig-discover") with the recipe topic.
...
**1. Read existing vision/roadmap:**
   - `.jig/specs/vision.md` exists? → Read it.
     - Status CONFIRMED → check roadmap.
     - Status DRAFT → present vision for confirmation before proceeding.
   - `.jig/specs/roadmap.md` exists? → Read it. Find next pending `- [ ]` item.
...
**3. Create Vision & Roadmap:**
   a. **Vision**: Draft purpose, users, core components, constraints, success criteria.
      Write to `.jig/specs/vision.md` with Status: DRAFT.
   ...
   b. **Roadmap**: Decompose vision into `vision.min_roadmap_items`~`vision.max_roadmap_items`
      ...
      Write to `.jig/specs/roadmap.md` with Status: DRAFT.
      Present to user → confirm/adjust loop → Status: CONFIRMED.
...
```bash
jig recipe log {id} --phase scoping
```
...
**2. Scan existing context**:
   - **Read project-map.md** (at `.jig/specs/project-map.md`) for the
   ...
   - **Load affected layers' detail** from `.jig/specs/layers/{name}.md`.
...
**4. Save immediately**: Write scope to `.jig/specs/recipes/{id}/scope.md`
...
**7. Register with roadmap** (if roadmap exists):
   ...
   Read `.jig/specs/roadmap.md`, find the matching pending item, and add `(recipe: {id})`
...
**8. Log confirmation and transition phase**:
   ```bash
   jig recipe log {id} --phase research --action research --output scope.md --result "scope confirmed"
   ```
...
2. Set phase back to scoping: `jig recipe log {id} --phase scoping`
...
- Update `.jig/specs/vision.md` with changes, set Status: DRAFT, re-confirm
- Assess roadmap impact: which items are affected?
- Update `.jig/specs/roadmap.md` if items changed/added/removed
...
1. /research — investigate technology, best practices, libraries.
   Research is scoped by `.jig/specs/recipes/{id}/scope.md`.
2. **/jig-domain-model — define entities, invariants (single owner),
...
3. **/jig-architect — propose ≥2 alternative decompositions, debate
   them, adjudicate, and commit the winner as the
   `<!-- architect-decision -->` block. CANNOT skip —
   `jig verify wireframe.md` raises missing_architect_decision_block
...
4. /jig-wireframe — design structure referencing `domain.md` entities,
   honoring the committed architect decision. Component
   responsibilities MUST honor the invariant owners declared in
   domain.md § 2. After saving, run `jig verify wireframe.md` and fix
...
2. **/jig-domain-model** — model the ADDED or CHANGED domain pieces.
   If existing domain docs live in `.jig/specs/layers/{name}.md`, load
   them and add only the delta for this recipe.
3. **/jig-architect** — same as above: ≥2 alternatives against the
   delta domain model, commit the decision block.
4. /jig-wireframe — design structure changes honoring domain.md
   invariants and the architect decision. Run `jig verify wireframe.md`
   after saving.
...
skills and broke `jig-verification-protocol.md`'s "never verify your
own output in the same context" rule. It is now split by responsibility:

- **Mechanical checks** run via `jig verify draft.md`:
...
- **Semantic checks** stay in fork context (separate Claude instance):
  - Contradiction detection, naming consistency, error-handling
    uniformity → `/jig-verify`
  - Completeness, missing cases, branch coverage → `/jig-audit`
  - Behavioral gaps, cross-boundary scenarios → `/jig-simulate`

After saving draft.md, run `jig verify .jig/specs/recipes/{id}/draft.md`
for the mechanical pass, then run `/jig-verify` for the semantic pass.
Do NOT re-implement either set inline here.

Also apply after every IMPROVE step, before the next /jig-verify run.

### ASSESS Decision Tree

The decision tree is **single-sourced in `jig-assess/SKILL.md`**. After each
`/assess`, read the `<jig-decision>` machine-readable block from the assess
output and execute the named action. The block's `phase` field is the next
phase to record:

```bash
jig recipe log {id} --phase <phase from <jig-decision>>
```

Action enum (see jig-assess § Part A for authoritative list): RESEARCH,
...
place prevents the drift that historically produced conflicting spec
and assess behavior.
...
1. **Every document modification → /verify.** No exceptions.
   ...
   enforced by `jig recipe log`, not by counting rounds yourself: after
   ...
   loop there and ask the user. See `jig-verification-protocol.md §
   Convergence`.
1a. **Round scope.** The first verification of a document is a full
   ...
   hook blocks `<jig>DONE</jig>` otherwise. When in doubt, run full.
1b. **Findings carry forward.** Each verify round receives the previous
   ...
   with `jig recipe findings list {id} --open`.
...
3b. **Minor handling split.** /verify and /audit tag minors as `[resolvable]`
   or `[deferred]` per `jig-verification-protocol.md § Severity Classification`:
   ...
   - `[deferred]` minors → append to a "## Known Uncertainties" section at the
     ...
     verbatim — the stop hook and /jig-implement Step 5.7 parse exactly
   ...
   - Loop exit: when `/verify` shows ONLY `[deferred]` minors, `/jig-assess`
     ...
     carry into `/jig-implement` as a watch-list.
4. **/simulate early**: Run after the FIRST verify cycle that produces critical=0.
   ...
   - Once critical=0, /simulate belongs in the SAME concurrent batch as
     /jig-verify and /jig-audit (loop protocol step 5) — all three read
...
After each action:
- **Changelog**: `jig recipe log {id} --action [type] --output [path]`
...
0. **Confirm the last round was a full pass.** If the most recent verify
   entry for draft.md used `--scope delta`, run one more /jig-verify at
...
5. Tell the user (plaintext, after the marker):
   > **Spec complete** — `{id}` spec finalized.
   > Next: run `/jig-implement {id}` to start implementation.

> **Checkpoint**: Spec finalized. Proceed directly to output `<jig>DONE</jig>`.
...
not the complete HOW. Implementation happens in `/jig-implement`.

## Recovery: `recipe.json` stuck mid-phase

If a session ended before `<jig>DONE</jig>` was emitted, the stop hook
did not fire and `recipe.json` may still show a mid-spec phase
(e.g. `phase=simulate, level=0, iteration=0`) even though
`verify-log.jsonl` already shows `converged`. Recover with:

```bash
jig recipe reconcile <recipe-id>             # prompts dry-run first
jig recipe reconcile <recipe-id> --dry-run   # preview plan
```

Reconcile is idempotent, spec-only (never touches implement-phase
