...
1. `domain.md` exists and passes `jig verify domain.md` (invariant
   ownership clean)
2. `scope.md` Status: CONFIRMED
3. `research/v1.md` exists AND contains an `## Official Guidance`
   section (per jig-research Step 3.5). If the section is missing,
   ...
   recommended architecture per `.claude/rules/jig-evidence-policy.md`
...
- **Node list** — files or modules with a ONE-SENTENCE responsibility
  (same "and"/"&"/"및" rule as jig-wireframe Step 1). Include file path
...
Run `Skill("jig-debate")` with topic:
...
Run `Skill("jig-adjudicate")`. Outcome possibilities:
...
```bash
# Via jig CLI (future: dedicated flag). For now, edit manifest.json
...
```bash
jig recipe log {id} --phase architect --action architect \
...
> `/jig-wireframe` — the component diagram now has a chosen
...
1. Write a minimal `<!-- architect-decision -->` block with
   ...
   The `Basis:` line is required — `jig verify` checks for it.
2. Still record invariant ownership — skipping the debate does NOT
   skip the ownership contract (`jig verify` cross-checks the mapping
   against domain.md § 2).
