...
- If first word is `--mid-run` → **mid-run mode** (Phase 11).
  ...
  Output goes to `.jig/specs/recipes/{id}/reviews/midrun-{timestamp}.md`,
...
- Read `.jig/specs/project-map.md` for layer structure.
- Read `.jig/specs/layers/{name}.md` for the relevant layer's patterns.
- If inside a recipe:
  ...
  - Run `jig graph --import --recipe {id}` to extract the actual import
...
Save to `.jig/specs/recipes/{id}/review.md` if inside a recipe.
...
```bash
jig recipe log {id} --action review --output review.md --result "N critical, N major, N dismissed"
```
...
When invoked as `/jig-review --mid-run {window}`:
...
5. Write the output to:
   `.jig/specs/recipes/{id}/reviews/midrun-{ISO8601}.md`
   NOT to `review.md`. The terminal review at /jig-implement Step 5.5
...
7. Log the invocation:
   ```bash
   jig recipe log {id} --action midrun-review --output reviews/midrun-<ts>.md --result "N critical, N major, N dismissed"
   ```
...
tasks (see jig-implement SKILL.md Step 3).
