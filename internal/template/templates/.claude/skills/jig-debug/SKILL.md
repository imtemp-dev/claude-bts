...
Before starting:
1. List `.jig/specs/recipes/` → find related recipes (especially the one
...
7. If `.jig/specs/roadmap.md` exists, read it for project context
...
```bash
jig recipe create --type debug --topic "$ARGUMENTS"
```
...
`.jig/specs/recipes/{id}/perspectives.md`:
...
### 1.3 Design Intent
Read `.jig/specs/project-map.md` for the layer overview.
Load the affected layer's detail from `.jig/specs/layers/{name}.md`.
...
```bash
jig recipe log {id} --phase research --action research --output perspectives.md --result "6 perspectives collected"
```
...
```bash
jig recipe log {id} --phase research --action research --output perspectives.md --result "cross-reference: N hypotheses ranked"
```
...
```bash
jig recipe log {id} --action research --result "hypothesis: CONFIRMED | RECONSIDERED | DISPUTED | UNVALIDATED"
```

## Step 4: Draft Fix Spec

Based on the validated top hypothesis, create `.jig/specs/recipes/{id}/draft.md`:
...
Apply **Draft Self-Check** before saving (same checklist as spec).

```bash
jig recipe log {id} --phase draft --action draft --output draft.md
```

## Step 5: Simulate

Use Skill("jig-simulate") on draft.md:
...
```bash
jig recipe log {id} --phase debate --action debate --result "{consensus}"
```

## Step 7: Verify Loop

Run /jig-verify on the current draft:
...
```bash
jig recipe log {id} --phase verify --action verify
```
...
4. Tell the user (plaintext, after the marker):
   ...
   > Next: run `/jig-implement {id}` to apply the fix.
...
Use Skill("jig-implement") with arguments: {id}
