...
final output — the `<jig-decision>` block (Part A) plus the
human-readable rationale (Part B). The ORCHESTRATOR parses the block
and executes the action; you only decide.

Bash in this fork is ONLY for read-only commands (`jig verify`,
`jig recipe status`). Never run state-mutating jig commands (log,
create, finalize, …) or write files from this fork.

## Do not call this skill when state already answers

The orchestrator should run `jig recipe assess-precheck {id} --doc <doc>`
first. When that prints a `<jig-decision>`, the next action is determined
...
1. Run level assessment via jig binary:
   ```bash
   jig verify $ARGUMENTS
   ```
   This returns the current level score and missing criteria.
   If `$ARGUMENTS` is empty, resolve the active recipe via
   `jig recipe status` and target its draft:
   `jig verify .jig/specs/recipes/{id}/draft.md`
   (`jig verify` requires exactly one file argument).
...
2. Build situational awareness:
   ...
   - Run `jig recipe findings list {id} --open` for the findings that
...
4. Decide the next action based on assessment:
   ...
   **Structural prerequisites — check FIRST, in this order** (a draft
   ...
   actions below):
   - `domain.md` missing, or `jig verify domain.md` reports
     critical/major (spec/design recipes) → action `DOMAIN_MODEL`,
     phase `domain-model`
   - `wireframe.md` lacks the `<!-- architect-decision -->` block
     → action `ARCHITECT`, phase `architect`
   - `wireframe.md` missing, or `jig verify wireframe.md` reports
   ...
   **If only [deferred] minors remain** → recommend FINALIZE
   ...
   in the draft's "## Known Uncertainties" section (per spec rule 3b).
   Recommendation: "Level 3 achieved with N runtime-deferred watch items.
   Run /jig-sync-check, then finalize. Deferred items will be validated
   during /jig-implement's test/simulate loop."

5. Output your assessment in TWO parts:

   **Part A — Machine-readable decision block (REQUIRED, exact format):**

   Emit this block verbatim, with valid JSON inside. The spec loop
   and `jig validate` parse this block; anything outside the block is
   ...
   `phase` MUST be a valid recipe phase from `jig-schema.md`.
...
- Consider what has already been done (check .jig/specs/recipes/{id}/ for previous research, debates, simulations).
