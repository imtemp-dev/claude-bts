...
在规范阶段，bts 迭代文档——探索意图、调研代码库、编写蓝图，并在独立的 AI 上下文中进行多轮验证。此阶段发现的错误只需文本编辑即可修复。

**蓝图不是转录。** 它只承载代码无法廉价证伪的部分——每条不变式以及维护它的文件、跨越每个边界的内容、不可逆的顺序及其回滚方式，以及能够证伪每条不变式的测试名称。函数签名、类型定义和边界情况表被有意排除：编译器免费生成这些内容，而用散文争论它们每次要花一轮验证，并留下下一轮还得重新检查的断言。

验证循环有上限——轮次上限和无进展预算。触及任何一个，剩余问题就移入 `## Known Uncertainties` 并开始实现。运行中的代码几秒钟就能解决它们。
...
- **文档优先** — 迭代规范，而非代码
- **蓝图不是转录** — 文档只保留代码无法廉价证伪的部分
- **循环有上限** — 不收敛时把剩余问题移入 Known Uncertainties 并开始实现
- **禁止自我验证** — 验证使用独立的代理上下文
